...
class MTNODCODetector(SemiBaseDetector):
    ...
    def loss(self, multi_batch_inputs: Dict[str, Tensor],
        ...
        if self.epoch < self.semi_train_cfg.get('warmup_step', -1):
            ...
            for i in range(len(batch_data_samples_unsup_student)):
                gt_data_sample, pred_data_sample = self.assign_pseudo_gt(batch_data_samples_unsup_student[i], multi_batch_data_samples['strong_student'][i])
                # 可信度小于阈值，加入到忽略样本中
                ...
                # 可信度小于阈值，则以预测标签作为训练标签
                for j in range(len(gt_data_sample.gt_instances.scores)):
                    if gt_data_sample.gt_instances.scores[j] <= threshold_clean:
                        gt_data_sample.gt_instances.labels[j] = pred_data_sample.labels[j]
                        gt_data_sample.gt_instances.bboxes[j] = pred_data_sample.bboxes[j]
                        # if self.epoch == self.num_epochs - 1:
                        #     self.save_correction(multi_batch_data_samples['strong_student'][i], gt_data_sample)
                gt_data_samples.append(gt_data_sample)
                if self.epoch == self.num_epochs - 1:
                    self.save_correction(multi_batch_data_samples['strong_student'][i], gt_data_sample)
            ...
            # print('studentbox:')
            # print(multi_batch_data_samples['strong_student'])
            # print('teacherbox')
            # print(multi_batch_data_samples['weak_teacher'])
            # print('pred')
            # print(gt_data_samples)
            ...
            losses.update(**self.loss_by_gt_instances(
            multi_batch_inputs['strong_student'], gt_data_samples))
        return losses
    ...
    # 将教师模型预测框、标注框、置信分数保存到json中
    def save_correction(self, origin: SampleList, corrected: SampleList):
        
        row = {
            "homography_matrix_1": origin.homography_matrix,
            "flip_direction_1": origin.flip_direction,
            "ori_shape_1": origin.ori_shape,
            "pad_shape_1": origin.pad_shape,
            "img_id_1": origin.img_id,
            "img_path_1": origin.img_path,
            "flip_1": origin.flip,
            "batch_input_shape_1": origin.batch_input_shape,
            "scale_factor_1": origin.scale_factor,
            "bboxes_1": origin.gt_instances.bboxes,
            "bboxes_true_1": bbox_project(
                origin.gt_instances.bboxes,
                torch.from_numpy(origin.homography_matrix).inverse().to(
                    self.data_preprocessor.device), origin.ori_shape),
            "labels_1": origin.gt_instances.labels,
            "homography_matrix_2": corrected.homography_matrix,
            "flip_direction_2": corrected.flip_direction,
            "ori_shape_2": corrected.ori_shape,
            "pad_shape_2": corrected.pad_shape,
            "img_id_2": corrected.img_id,
            "img_path_2": corrected.img_path,
            "flip_2": corrected.flip,
            "batch_input_shape_2": corrected.batch_input_shape,
            "scale_factor_2": corrected.scale_factor,
            "bboxes_2": corrected.gt_instances.bboxes,
            "bboxes_true_2": bbox_project(
                corrected.gt_instances.bboxes,
                torch.from_numpy(corrected.homography_matrix).inverse().to(
                    self.data_preprocessor.device), corrected.ori_shape),
            "scores_2": corrected.gt_instances.scores,
            "labels_2": corrected.gt_instances.labels,
        }
        # 将字典中的numpy数组转换为列表
        row = self.convert_numpy(row)
        # 检查文件是否存在且不为空
        if os.path.exists('correct_save_correction.json') and os.path.getsize('correct_save_correction.json') > 0:
            with open('correct_save_correction.json', 'r') as file:
                # 读取现有数据
                data = json.load(file)
                # 确保读取的数据是列表格式，以便我们可以追加数据
                if not isinstance(data, list):
                    raise ValueError("JSON root element must be a list for appending data.")
                data.append(row)
        else:
            # 如果文件不存在或为空，创建一个新列表
            data = [row]
        ...
        # 将更新后的数据写回JSON文件
        with open('correct_save_correction.json', 'w') as file:
    ...
    def assign_pseudo_gt(self, pred: SampleList,
        ...
        return_pred = InstanceData()
        return_pred.scores = pred.gt_instances.scores[nonzero_elements]
        return_pred.labels = pred.gt_instances.labels[nonzero_elements]
        return_pred.prob_scores = pred.gt_instances.prob_scores[nonzero_elements]
        return_pred.bboxes = pred.gt_instances.bboxes[nonzero_elements]
