...
from mmdet.models.utils import (filter_scores_and_topk, images_to_levels, multi_apply, unpack_gt_instances,
...
class NODHead(ATSSHead):
    ...
    def loss_by_feat_single(self, anchors: Tensor, cls_score: Tensor,
        ...
        # anchors经过multi_apply被拆分成5个:[4, 13456, 4], [4, 3364, 4], [4, 841, 4], [4, 225, 4], [4, 64, 4]，reshape之后变为[53824,4]...
        anchors = anchors.reshape(-1, 4)
        # cls_scores经过multi_apply被拆分成5个:[4, 10, 116, 116], [4, 10, 58, 58], [4, 10, 29, 29], [4, 10, 15, 15], [4, 10, 8, 8]，reshape之后变为[53824,10]，代表某一个level上4个batch的所有anchor点
        cls_score = cls_score.permute(0, 2, 3, 1).reshape(
            -1, self.cls_out_channels).contiguous()
        # bbox_pred经过multi_apply被拆分成5个:[4, 4, 92, 92], [4, 4, 46, 46], [4, 4, 23, 23], [4, 4, 12, 12], [4, 4, 6, 6]，reshape之后变为[33856,4]...
        bbox_pred = bbox_pred.permute(0, 2, 3, 1).reshape(-1, 4)
        # bbox_targets经过multi_apply被拆分成5个:[4, 13456, 4], [4, 3364, 4], [4, 841, 4], [4, 225, 4], [4, 64, 4]，reshape之后变为[53824,4]...
        bbox_targets = bbox_targets.reshape(-1, 4)
        # labels经过multi_apply被拆分成5个:[4, 13456], [4, 3364], [4, 841], [4, 225], [4, 64]，reshape之后变为[53824]...
        labels = labels.reshape(-1)
        # alignment_metrics跟labels格式一样
    ...
    def loss_by_feat(
        ...
        Args:
            cls_scores (list[Tensor]): Box scores for each scale level
                Has shape (N, num_anchors * num_classes, H, W) 格式为([batch, 10, 116, 116], [batch, 10, 58, 58], [batch, 10, 29, 29], [batch, 10, 15, 15], [batch, 10, 8, 8])
            bbox_preds (list[Tensor]): Decoded box for each scale
                level with shape (N, num_anchors * 4, H, W) in
                [tl_x, tl_y, br_x, br_y] format. 格式为([batch, 4, 116, 116], [batch, 4, 58, 58], [batch, 4, 29, 29], [batch, 4, 15, 15], [batch, 4, 8, 8])
            batch_gt_instances (list[:obj:`InstanceData`]): Batch of
                gt_instance.  It usually includes ``bboxes`` and ``labels``
                attributes.
            batch_img_metas (list[dict]): Meta information of each image, e.g.,
                image size, scaling factor, etc. 格式为[batch, 图片信息（包括尺寸路径等）]
        ...
        Returns:
            dict[str, Tensor]: A dictionary of loss components.
        """
        num_imgs = len(batch_img_metas) # batch数量
        featmap_sizes = [featmap.size()[-2:] for featmap in cls_scores]
        assert len(featmap_sizes) == self.prior_generator.num_levels

        device = cls_scores[0].device
        # 对feature map上的每一个点得到对应的anchor，格式为
        # [[torch([13456, 4]),      116^2
        #   torch([3364, 4]),       58^2
        #   torch([841, 4]),        29^2
        #   torch([225, 4]),        15^2
        #   torch([64, 4])]*batch]  8^2
        anchor_list, valid_flag_list = self.get_anchors(
            featmap_sizes, batch_img_metas, device=device)

        # 将所有level的anchor的标签拼接到一起，即13456+3364+841+225+64，格式为torch[4, 17950, 10]
        flatten_cls_scores = torch.cat([
            cls_score.permute(0, 2, 3, 1).reshape(num_imgs, -1,
                                                  self.cls_out_channels)
            for cls_score in cls_scores # 循环每个level
        ], 1)
        # 将所有level的anchor的box拼接到一起，即13456+3364+841+225+64，格式为torch[4, 17950, 4]，乘stride是为了还原成本身的大小
        flatten_bbox_preds = torch.cat([
            bbox_pred.permute(0, 2, 3, 1).reshape(num_imgs, -1, 4) * stride[0]
            for bbox_pred, stride in zip(bbox_preds,
                                         self.prior_generator.strides)
        ], 1)
        # self.prior_generator.strides: [(8, 8), (16, 16), (32, 32), (64, 64), (128, 128)]

        # 返回cls_reg_targets包含anchors_list, labels_list, label_weights_list, bbox_targets_list, norm_alignment_metrics_list，分别为每一level的值
        # 其中anchors_list为[[4,13456,4],[4,3364,4],[4,841,4],[4,225,4],[4,64,4]]代表每一个level每一个batch的anchor
        # norm_alignment_metrics_list为[[4,13456],[4,3364],[4,841],[4,225],[4,64]]，代表每一level每一个batch每一个anchor处的分数
        ...
        # losses_cls: [80.4997, 0.0016, 0.0314, 0.1247, 0.0087]
        # losses_bbox: [40.1271, 0, 0, 0, 0]
        # cls_avg_factors: [53, 0, 0, 0, 0] 对齐分数
        # bbox_avg_factors: [31.8600, 0, 0, 0, 0] centerness分数
    ...
    def loss_by_feat_momentum(
            self,
            cls_scores: List[Tensor],
            bbox_preds: List[Tensor],
            cls_scores_momentum: List[Tensor],
            bbox_preds_momentum: List[Tensor],
            batch_gt_instances: InstanceList,
            batch_img_metas: List[dict],
            batch_gt_instances_ignore: OptInstanceList = None) -> dict:
        """Calculate the loss based on the features extracted by the detection
        ...
        flatten_cls_scores_momentum = torch.cat([
            cls_score_momentum.permute(0, 2, 3, 1).reshape(num_imgs, -1,
                                                  self.cls_out_channels)
            for cls_score_momentum in cls_scores_momentum # 循环每个level
        ], 1)
        # 将所有level的anchor的box拼接到一起，即13456+3364+841+225+64，格式为torch[4, 17950, 4]，乘stride是为了还原成本身的大小
        flatten_bbox_preds = torch.cat([
            bbox_pred.permute(0, 2, 3, 1).reshape(num_imgs, -1, 4) * stride[0]
        ...
        flatten_bbox_preds_momentum = torch.cat([
            bbox_pred_momentum.permute(0, 2, 3, 1).reshape(num_imgs, -1, 4) * stride[0]
            for bbox_pred_momentum, stride in zip(bbox_preds_momentum,
                                         self.prior_generator.strides)
        ], 1)
        # self.prior_generator.strides: [(8, 8), (16, 16), (32, 32), (64, 64), (128, 128)]
        ...
        cls_reg_targets = self.get_targets(
            flatten_cls_scores,
            flatten_bbox_preds,
            flatten_cls_scores_momentum,
            flatten_bbox_preds_momentum,
            anchor_list,
            valid_flag_list,
            batch_gt_instances,
            batch_img_metas,
    ...
    def get_targets(self,
                    ...
                    cls_scores_momentum: List[List[Tensor]],
                    bbox_preds_momentum: List[List[Tensor]],
        ...
        # 每个level的anchor数量[13456, 3364, 841, 225, 64]
        num_level_anchors = [anchors.size(0) for anchors in anchor_list[0]]
        # [[13456, 3364, 841, 225, 64], [13456, 3364, 841, 225, 64], [13456, 3364, 841, 225, 64], [13456, 3364, 841, 225, 64]]
        num_level_anchors_list = [num_level_anchors] * num_imgs

        # concat all level anchors and flags to a single tensor
        # anchor_list将所有level拼接后更新为torch.Size([17950, 4])
        ...
        # initial_epoch之前采用ATSS算法的正样本分配策略，直接拿anchor分配正负样本
        # all_anchors为[17950, 4]*4(batch)基本仍为原始anchor_list；all_labels为[17950]*4(batch)代表每个anchor的label；all_label_weights为[17950]*4(batch)代表每个label的权重；all_bbox_targets为[17950, 4]*4(batch)代表分配的每个anchor的gt_box；all_bbox_weights为[17950, 4]*4(batch)，正样本位置的bbox权重为1其余为0
        ...
        # 采用tood设计的正样本分配策略，利用预测的cls和bbox分配正负样本
        # all_assign_metrics为[17950]*4(batch)，正样本位置有tood分数其余为0
        else:
            (all_anchors, all_labels, all_label_weights, all_bbox_targets,
    ...
    def _get_targets_single(self,
                            cls_scores: Tensor,
                            bbox_preds: Tensor,
                            cls_scores_momentum: Tensor,
                            bbox_preds_momentum: Tensor,
                            flat_anchors: Tensor,
                            valid_flags: Tensor,
        ...
        pred_instances = InstanceData(
            ...
            scores_momentum=cls_scores_momentum[inside_flags, :],
            bboxes_momentum=bbox_preds_momentum[inside_flags, :])
        ...
        # map up to original set of anchors，补全为原始的shape
        if unmap_outputs:
    ...
    def NODloss(self, outs: Tuple[Tuple[Tensor]], outs_momentum: Tuple[Tuple[Tensor]], batch_data_samples: SampleList) -> dict:
        ...
        loss_inputs = outs + outs_momentum + (batch_gt_instances, batch_img_metas,
                              batch_gt_instances_ignore)
        losses = self.loss_by_feat_momentum(*loss_inputs)
        return losses
