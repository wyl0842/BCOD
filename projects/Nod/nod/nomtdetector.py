...
class NOMTDetector(SemiBaseDetector):
    ...
    def loss(self, multi_batch_inputs: Dict[str, Tensor],
        ...
        if self.epoch < self.semi_train_cfg.get('warmup_step', -1):
            ...
            for i in range(len(batch_data_samples_unsup_student)):
                ...
                for i in range(len(gt_data_sample.gt_instances.scores)):
                    if gt_data_sample.gt_instances.scores[i] <= threshold_clean:
                        gt_data_sample.gt_instances.labels[i] = pred_data_sample.labels[i]
                        gt_data_sample.gt_instances.bboxes[i] = pred_data_sample.bboxes[i]
                gt_data_samples.append(gt_data_sample)
    ...
    def get_pseudo_instances(
        ...
        """Get pseudo instances from student model."""
        self.student.eval()
        feat = self.student.extract_feat(batch_inputs)
        results_list = self.student.bbox_head.predict_prob(feat, batch_data_samples, rescale=False)
        batch_info = {}
    ...
    def save_correction(self, origin: SampleList, corrected: SampleList):
        ...
        header = not pd.os.path.exists('save_correction.csv')
        batch_df.to_csv('save_correction.csv', mode='a', index=False, header=header)
