...
class MTCODetector(SemiBaseDetector):
    ...
    def loss(self, multi_batch_inputs: Dict[str, Tensor],
        ...
        if self.epoch < self.semi_train_cfg.get('warmup_step', -1):
            ...
            for i in range(len(batch_data_samples_unsup_student)):
                gt_data_sample, pred_data_sample = self.assign_pseudo_gt(batch_data_samples_unsup_student[i], multi_batch_data_samples['weak_teacher'][i])
                # # 可信度小于阈值，加入到忽略样本中
