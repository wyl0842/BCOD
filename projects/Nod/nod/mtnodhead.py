...
from mmdet.models.utils import (filter_scores_and_topk, images_to_levels, multi_apply, unpack_gt_instances,
                     sigmoid_geometric_mean, unmap, select_single_mlvl)
from mmdet.models.dense_heads.atss_head import ATSSHead
from .utils import my_filter_scores_and_topk
from .utils_nms import my_batched_nms
...
class MTNODHead(ATSSHead):
    ...
    def _predict_by_feat_single(self,
        ...
        for cls_score, bbox_pred, priors, stride in zip(
            ...
            # find a slight drop in performance, you can set a larger
            # `nms_pre` than before.
            results = my_filter_scores_and_topk(
                scores, cfg.score_thr, nms_pre,
                dict(bbox_pred=bbox_pred, priors=priors))
            scores, labels, keep_idxs, filtered_results = results
    ...
    def predict_prob(self,
        ...
        predictions = self.predict_by_feat_prob(
            *outs, batch_img_metas=batch_img_metas, rescale=rescale)
        return predictions
    
    def predict_by_feat_prob(self,
                        cls_scores: List[Tensor],
                        bbox_preds: List[Tensor],
        ...
        for img_id in range(len(batch_img_metas)):
            ...
            results = self._predict_by_feat_single_prob(
    ...
    def _predict_by_feat_single_prob(self,
                                cls_score_list: List[Tensor],
                                bbox_pred_list: List[Tensor],
                                score_factor_list: List[Tensor],
                                mlvl_priors: List[Tensor],
        ...
        return self.my_bbox_post_process(
            results=results,
            cfg=cfg,
            rescale=rescale,
            with_nms=False,
    ...
    def my_bbox_post_process(self,
        ...
        if with_nms and results.bboxes.numel() > 0:
            ...
            # det_bboxes, keep_idxs = my_batched_nms(bboxes, results.scores,
            #                                     results.labels, cfg.nms)
            det_scores, det_bboxes, keep_idxs = my_batched_nms(bboxes, results.scores,
                                                results.labels, None)
            results = results[keep_idxs]
            # some nms would reweight the score, such as softnms
            results.scores = det_bboxes[:, -1]
            results.prob_scores = det_scores
